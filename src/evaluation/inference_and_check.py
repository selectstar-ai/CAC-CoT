...
def perform_inference_and_check(handler: TaskHandler, temperatures, max_tokens, result_file, llm, system_prompt, args):
    ...
    def get_conversation_prompt_by_messages(tokenizer, messages):
        ...
        if args.add_think_tok != '':
            text+=args.add_think_tok
        return text
    ...
    for temp in temperatures:
        ...
        if args.model.startswith("openai"):
            ...
            prompts = [get_conversation_prompt_by_messages(tokenizer, conversation) for conversation in conversations]
            print("### Sample:")
            print(prompts[-1])
            print("-----")
...
def main():
    ...
    parser.add_argument("--dataset", type=str, required=True, choices=["NUMINA", "APPS", "TACO", "MATH500", "AIME24", "AIME25", "AMC23", "GPQADiamond", "MMLU", "MMLUPro", "LiveCodeBench", "GSM8K", "ARC-C"], help="Dataset to process.")
    parser.add_argument("--model", type=str, required=True, default="Qwen/QwQ-32B-Preview", help="The model to run.")
    parser.add_argument("--tp", type=int, default=8, help="Tensor Parallelism Degree")
    parser.add_argument("--max_tokens", type=int, default=16384, help="Max tokens for the model.")
