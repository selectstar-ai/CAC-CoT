...
class MathTaskHandler(TaskHandler):
    ...
    def generate_prompt(prompt, model):
        if "CAC" in MODEL_TO_NAME[model]:
            # return "Return your answer in the form: `Final Answer: (your answer)` " + prompt
            return prompt
...
class AMC23TaskHandler(MathTaskHandler):
    def __init__(self):
        self.dataset = "math-ai/amc23"
    
    @staticmethod
    def generate_prompt(prompt, model):
        if MODEL_TO_NAME[model] == "Sky-T1-32B-Preview":
            return prompt + "\nReturn your final response within \\boxed{{}}"
        elif "CAC" in MODEL_TO_NAME[model]:
            # return prompt + "\nReturn your answer in the form: `Final Answer: (your answer)`"
            # return prompt + "\nReturn your final response within \\boxed{{}}"
            return prompt
        else:
            return prompt + "\nReturn your final response within \\boxed{{}}"
    ...
    def make_conversations(self, data, system_prompt, model=None):
        conversations = []
        for problem in data:
            prompt_text = self.generate_prompt(problem["question"], model)
            conversations.append([
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": prompt_text},
            ])
    ...
    def load_and_filter_dataset(self, start, end, split="train", source=None, filter_difficulty=False, args=None):
        ...
        dataset = dataset.add_column('problem', dataset['question'])
        train_data = dataset.to_pandas()
        return train_data.iloc[start:end] if end > 0 else train_data.iloc[start:]
    
class AIME25TaskHandler(MathTaskHandler):
    def __init__(self):
        self.dataset = "math-ai/aime25"
...
class AIME24TaskHandler(MathTaskHandler):
    def __init__(self):
...
class GPQADiamondTaskHandler(TaskHandler):
    ...
    def generate_prompt(prompt, model):
        if "CAC" in MODEL_TO_NAME[model]:
            # return "Return your answer in the form: `Final Answer: (A, B, C, or D)` and only include the letter choice (A, B, C, or D) as your final response. " + prompt
            # return "only include the letter choice (A, B, C, or D) as your final response. " + prompt
...
class GSM8KTaskHandler(TaskHandler):
    ...
    @staticmethod
    def generate_prompt(prompt, model):
        if 'datumo' in MODEL_TO_NAME[model]:
            return prompt
        else:
            return prompt + "\nReturn your final response within \\boxed{{}}"
...
TASK_HANDLERS = {
    ...
    "AIME24": AIME24TaskHandler,
    "AIME25": AIME25TaskHandler,
